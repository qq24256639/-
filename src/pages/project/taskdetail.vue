<template>
  <el-dialog :visible.sync="showingTaskDetail" @closed="afterClose" width="58.3%">
    <div slot="title">
      <div class="task-detail-header">
        ...
        <div class="task-detail-header-right">
          ...
          <a href="javascript:;" @click='childrenTaskShow'><icon-svg icon-class="gengduo-2"></icon-svg>子任务</a>
    ...
    <!-- 子任务添加 -->
    <div class="task-detail-participant-wrapper" ref='chilrenTaskDiv' style='padding:10px 0;' v-show='testData.length !== 0 || showIcon == true' >
      <label class='modal-row-label ng-binding'>
        <a href="javascript:;" ><icon-svg icon-class="zirenwu"></icon-svg>子任务</a>
      </label>
            ...
            <div class="entry-header">
              <span class="entry-title">
                <span class="completed-count">
                  <b>{{ getCompletedTasks(testData.id).length}}</b>/{{ getTasks().length }}
                </span>
              </span>
              <div class="progressbar-container">
                <div class="progress">
                  <div class="progress-bar"
                       :style="{width: (getPercent(testData.id) < 100 ? getPercent(testData.id) : 100) + '%'}"
                       style="width: 100%;"></div>
                </div>
              </div>
            </div>
            <!-- 任务列表 -->
            <div class="entry-tasks entry-tasks-chidren" >
                <div v-for="(task, index) in testData"
                     :key="index"
                     class="task-card">
                  <div class="task-main" >
                    <a href="javascript:;" @click.stop="checkTaskChilren(task.tiFkNo,index)" class="task-check">
                      <icon-svg v-bind:icon-class="task.tiStatus == 2 ? '_checkbox_click' : 'checkbox-no'"/>
                    </a>
                    <span class="task-title" @click.stop='taskDetailShowing(task.tiFkNo,index)' v-show='task.ture === false'>{{ task.tiTitle }}</span>
                    <textarea class="form-control taskName update-sub-task-name" refs='childrenTextarea' v-model="task.tiTitle" :placeholder="task.name" v-show='task.ture === true' @keypress.enter.prevent="editTitleAddChilrenTask(task.tiFkNo,index)"></textarea>
                    <div class='editDelIcan'  v-show='task.ture === false'>
                      <a href="javascript:;" class="childrenTaskEdit" @click.stop='childrenTaskEdit(index)' ><icon-svg icon-class="xiugai"></icon-svg></a>
                      <a href="javascript:;" class="childrenTaskDel" @click.stop='childrenTaskDel(task.tiFkNo)'><icon-svg icon-class="shanchu"></icon-svg></a>
                    </div>
                  </div>
                </div>
            </div>
            <div class=" task-card entry-tasks-chidren" v-if='newChilrenTaskIf == true' >
              <a href="javascript:;" class="btn-add" @click="doAddChilrenTask()">
                <icon-svg icon-class="add"></icon-svg>
                添加新任务
              </a>
            </div>
            <div class="add-composer" v-else>
                <textarea
                  v-model='newChilrenTask'
                  class="form-control taskChilrenName"
                  ref="newChilrenTask"
                  @keypress.enter.prevent="submitAddChilrenTask()"            
                  placeholder="添加子任务，按回车键(Enter)保存"
                  style="overflow: hidden; word-wrap: break-word; resize: none; height:32px;line-height:12px;">
                </textarea>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    checkTaskChilren(id,index){   // 子任务 对号切换
      const status = this.testData[index].tiStatus === '2' ? '1' : '2'
      var that = this;
      this.$http.post('/api/mytask/modifyTaskInfo',{
        "tiFkNo":id,
        "tiStatus":status
    ...
    taskDetailShowing(task,id){
      var childrenData = {id:task,showTrue:false};
      this.$emit('closedTaskdetail',childrenData)
    },
    updateComment(){
        var that = this;
